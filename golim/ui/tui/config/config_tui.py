"""Textual configuration wizard for ``golim -init``.

This mirrors the original interactive flow in ``golim.config.basic``, but
...
class ConfigApp(ConfigUIMixin, App[int]):
    ...
    def on_mount(self) -> None:
        ...
        # self._append_log("golim configuration", STYLE_DIM)
...
def init_command_tui(binary: str = "ollama") -> int:
    ...
    Falls back to the plain ``golim.config.basic.init_command`` flow when
    Textual is unavailable, so ``golim --init`` keeps working in minimal
    environments.
    """
    try:
        ...
        from golim.ui.basic.basic_config import init_command
