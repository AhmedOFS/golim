"""Basic terminal configuration UI for golim (interactive init flow)."""
import os
import subprocess
import time
...
def init_openrouter(config: Config) -> int:
    """Configure golim to use OpenRouter."""
...
def init_ollama(config: Config, binary: str) -> int:
    """Configure golim to use Ollama (local)."""
...
def init_openai_compatible(config: Config) -> int:
    """Configure golim to use an OpenAI-compatible server."""
...
def init_command(binary: str = "ollama") -> int:
    """Initialize golim by detecting Ollama and selecting a model."""
    config = get_config()
