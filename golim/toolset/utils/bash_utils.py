"""Bash command execution for golim — restricted and unrestricted modes."""
...
def _privileged_list_notice_payload(command: str) -> dict:
    message = (
        "Privileged commands are routed through the golim wrapper; "
...
def _exec_shell(
    ...
    if PRIVILEGED_WRAPPER in prepared:
        ...
        if not token:
            # Fail closed: no session token available, so golim-authd
