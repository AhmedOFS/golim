"""Session-token client for golim-authd.

The privileged wrapper runs through a NOPASSWD sudoers rule and gates
access on a session token issued by the root-side golim-authd daemon.
...
password with golim-authd (which verifies it via sudo/PAM as the user),
...
in logs; golim-authd stores only its SHA-256 digest.
"""
...
from golim.toolset.vars import PRIVILEGED_WRAPPER

AUTHD_SOCKET_PATH = "/run/golim/authd.sock"
...
def _authd_request(method, params):
    """Send one request to golim-authd and return its result."""
    ...
    try:
        ...
        while b"\n" not in buf:
            ...
            if not chunk:
                raise ConnectionError("No response from golim-authd")
            buf += chunk
...
def register_session(password: str) -> str | None:
    """Verify the password with golim-authd and return the session token."""
...
def has_valid_session_token() -> bool:
    """Check the cached token against golim-authd without exposing it."""
    token = session_token()
