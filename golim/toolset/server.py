#!/usr/bin/env python3
"""
golim_server - Persistent, self-terminating MCP tool server.
...
from golim.toolset.vars import INACTIVITY_TIMEOUT_SECONDS
from golim.toolset.config import get_config
from golim.toolset import auth_session
from golim.toolset.utils.cancellation import bind_tool_cancellation
...
class MCPServer:
    ...
    async def _run_tool_call(
        ...
        def obtain_session_token():
            ...
            with golim-authd (which verifies it via sudo/PAM), and the
    ...
    async def handle_request(
        ...
        try:
            ...
            if method == "tools/list":
                ...
                # golim-authd, which verifies it via sudo/PAM and issues
...
def run_server():
    ...
    from golim.toolset.config import init_config

    resolve_app_home()
    init_config()
    from golim.toolset.tools import mcp

    print("Starting golim MCP tool server...")
