"""Runtime lifecycle for golim agent runs."""
...
from golim.core.skill_loader import SkillsLoader
...
class Runtime:
    ...
    def ensure_mcp_server(self):
        ...
        try:
            ...
            subprocess.Popen(
                [sys.executable, "-m", "golim.toolset.server"],
    ...
    def authenticate_sudo(self) -> None:
        ...
        The password is registered with the root-side golim-authd, which
        ...
        invocation. Skipped silently when the wrapper or golim-authd is
        ...
        if not status.get("authd_available", True):
            ui = self._active_ui()
            ui.message(
                "Privileged commands are unavailable: golim-authd is "
