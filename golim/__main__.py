...
def main(argv: list[str] | None = None) -> int:
    """Main entry point for golim CLI."""
    resolve_app_home()
    ...
    parser.add_argument(
        "-i", "--init",
        action="store_true",
        help="initialize golim (opens the configuration TUI)"
    )
    parser.add_argument(
        "-init",
        action="store_true",
        dest="init",
        help="initialize golim (alias for --init, opens the configuration TUI)"
    )
    parser.add_argument(
        "--binary",
        default="ollama",
