...
LAUNCHER_SOURCE = ROOT / "launcher" / "golim_launcher.c"
...
def flatten_site_packages(runtime: Path) -> None:
    ...
    for entry in sorted(site_packages.iterdir()):
        copy_entry(entry, (runtime / "lib" if entry.name == "golim" else runtime) / entry.name)
...
def _compiler() -> str:
    ...
    if compiler is None:
        raise RuntimeError("a C compiler is required to build the golim launcher")
    return compiler
...
def compile_linux_launcher(runtime: Path, *, output_name: str = "golim", entry_module: str = "golim.__main__", entry_function: str = "main") -> None:
    ...
    run([
        ...
        f'-DGOLIM_ENTRY_MODULE="{entry_module}"',
        f'-DGOLIM_ENTRY_FUNCTION="{entry_function}"',
...
def compile_macos_launcher(runtime: Path, *, output_name: str = "golim", entry_module: str = "golim") -> None:
    run([_compiler(), "-O2", "-DNDEBUG", f'-DGOLIM_ENTRY_MODULE="{entry_module}"', str(LAUNCHER_SOURCE), "-o", str(runtime / output_name)])
    (runtime / output_name).chmod(0o755)
...
def compile_application(runtime: Path) -> None:
    application = runtime / "lib" / "golim"
    if not application.is_dir():
        raise RuntimeError("flattened runtime does not contain golim")
    python = next(
...
def assemble_runtime(config: ReleaseConfig, platform_name: str) -> None:
    ...
    if platform_name == "linux":
        ...
        compile_linux_launcher(config.release_dir, output_name="golim-tools", entry_module="golim.toolset.server", entry_function="run_server")
    else:
        compile_macos_launcher(config.release_dir)
        compile_macos_launcher(config.release_dir, output_name="golim-tools", entry_module="golim.toolset.server")
