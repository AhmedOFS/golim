"""Unit tests for the root-side openterm token broker."""
...
BROKER_SOURCE = Path(__file__).resolve().parent.parent / "packaging" / "openterm-broker.py"


def _load_broker_module():
    spec = importlib.util.spec_from_file_location("openterm_broker", BROKER_SOURCE)
...
class BrokerRequestTests(unittest.TestCase):
    def setUp(self):
        ...
        self.broker = self.module.Broker(socket_path="/tmp/unused.sock")

    def tearDown(self):
        self.broker._register_failures.clear()
        self.broker._tokens.clear()

    def test_ping(self):
        result = self.broker.handle("ping", {}, peer_uid=1000)
    ...
    def test_register_rejects_invalid_request(self):
        for params in ({}, {"user": "ahmed"}, {"user": "ahmed", "password": ""}):
            result = self.broker.handle("register", params, peer_uid=1000)
            self.assertFalse(result["ok"], params)

    def test_register_rejects_wrong_password(self):
        with patch.object(self.broker, "_verify_password", return_value=False):
            result = self.broker.handle(
                "register", {"user": "ahmed", "password": "wrong"}, peer_uid=1000
            )
        self.assertFalse(result["ok"])
        self.assertEqual(self.broker._tokens, {})

    def test_register_mints_verifiable_and_revocable_token(self):
        with patch.object(self.broker, "_verify_password", return_value=True):
            registered = self.broker.handle(
        ...
        raw = repr(self.broker._tokens)
        self.assertNotIn(token, raw)

        revoked = self.broker.handle(
    ...
    def test_expired_tokens_stop_verifying(self):
        ...
        token = registered["token"]
        user_tokens = self.broker._tokens["ahmed"]
        ...
        self.assertNotIn(digest, self.broker._tokens.get("ahmed", {}))
    ...
    def test_unknown_method_reports_error(self):
        result = self.broker.handle("nope", {}, peer_uid=1000)
        self.assertFalse(result["ok"])
...
class BrokerPasswordVerificationTests(unittest.TestCase):
    ...
    def test_verify_password_child_keeps_pipe_read_end_open(self):
        ...
        def fake_run(*_args, **kwargs):
            ...
            # the broker cannot ever validate a password.
        ...
        with patch.object(self.module.pwd, "getpwnam", return_value=entry), \
            ...
            self.assertTrue(self.broker._verify_password(entry.pw_name, "pw"))
