...
- `openterm/core/runtime.py`: owns MCP client lifecycle, tool discovery, skill selection, interrupt state, and run state copied from the agent (`messages`, `execution_history`, `result`, `last_thinking_trace`). It starts `openterm-tools.service` with `systemctl --user start` when the socket is missing, falls back to launching `openterm.toolset.server` directly when user systemd is unavailable, retries tool discovery, optionally performs proactive sudo authentication according to config, and exposes `run()` plus `run_followup()`. Followups are user inputs starting with `//`: after a completed run they inject `followup: ...`; while a run is active the TUI queues them as an interrupt/elaboration and injects `clarification: ...`.
...
- `openterm/core/agent_events.py`: common UI protocol consumed by `ToolAgent`. It includes status, message, tool-call, tool-output, privileged approval, sudo-password auth, Python approval/display, and thinking-trace hooks.
...
- `openterm/toolset/server.py`: persistent MCP-like tool server over a Unix domain socket at `/tmp/openterm_tools_{username}.sock` using `pwd.getpwuid(os.getuid()).pw_name` with `$USER` fallback. It has a 20-minute inactivity shutdown and emits newline-delimited JSON frames for streaming output.
- `openterm/toolset/tools.py`: exposed MCP tool implementations. The registered tools are marked with `__mcp_tool__` and attached to the `mcp` object. Current exposed tool names include `finder`, `read_file`, `write_file`, `bash`, `exec` (backed by `exec_python`), `system_info`, and `websearch`. This is the source of truth; there is no `openterm/toolset/tools_mcp.py`.
- `openterm/toolset/utils/bash_utils.py`: restricted/unrestricted bash parsing and execution helpers, including command parsing, pipelines, `&&`, glob/env expansion, streaming, PTY streaming for apt/snap-style commands, output finalization, and privileged command routing.
- `openterm/toolset/config.py`: MCP configuration singleton and privileged-command whitelist state. It reads the nested openterm config, exposes MCP attributes and web-search credentials, and owns whitelist reads/updates. The whitelist path is `~/.openterm/privileged_whitelist`, overridable with `OPENTERM_PRIVILEGED_WHITELIST`.
- `openterm/toolset/vars.py`: MCP constants including `OUTPUT_LINE_LIMIT = 200`, file page size, wrapper path, parser restrictions, web-search endpoints, and server timeout.
- `openterm/toolset/utils/web_utils.py`: Exa/Parallel web search helpers. Provider is selected by `websearch_provider`, defaulting to Exa. API keys come from config or `EXA_API_KEY` / `PARALLEL_API_KEY`.
...
- `packaging/openterm-tools.service`: systemd user service template (`Type=simple`, `Restart=on-failure`) installed under `/usr/lib/systemd/user/` by the Debian package.
...
- `scripts/release/`: assembles relocatable application runtimes in `dist/linux` and `dist/macos`, including `openterm` and `openterm-tools` launchers.
...
- There is no `openterm/openterm_server.py`; use `openterm/toolset/server.py`.
- There is no `openterm/toolset/tools_mcp.py`; use `openterm/toolset/tools.py`.
- There is no `openterm/skills_loader/`; use `openterm/core/skill_loader.py`.
- There is no `openterm/toolset/utils/mcp_utils.py` or `openterm/toolset/utils/privilege.py`; use the singleton in `openterm/toolset/config.py`.
...
- Keep MCP server responses newline-delimited JSON. Streaming tool calls send zero or more `tools/progress` notifications (JSON-RPC frames with a `method` and no `id`, correlated by `params.token`) followed by one final `"result"` frame. A held privileged bash call may also send an `approval_request` frame, resolved out-of-band by a separate `approval/respond` request, or an `auth_request` frame when the session token is missing, resolved by `auth/respond` carrying the password for openterm-authd registration. Approval and auth exchanges are never returned to the model as a tool result, and passwords never appear in server logs (raw request text is redacted before logging).
- The privileged wrapper is covered by a wrapper-specific `NOPASSWD` sudoers rule, but it is not an unrestricted root escape: the root-side `openterm-authd` service verifies a startup sudo password and mints a random in-memory session token. The MCP server passes that token only through `OPENTERM_SESSION_TOKEN`; the wrapper verifies it with openterm-authd on every invocation and fails closed if the authd, token, or whitelist check fails. Passwords are never passed through command stdin, so ordinary pipes and PTY-backed apt/snap commands remain independent of authentication. Before any wrapper invocation the bash tool consults an injected `_session_token` provider; a cached token short-circuits, otherwise one out-of-band `auth_request` collects the password and registers it with openterm-authd.
...
- Socket path logic exists in multiple places. `openterm/toolset/server.py` uses UID/pwd lookup; `openterm/core/utils.py` is used by the client/runtime. Keep them compatible when changing.
...
- `openterm.toolset.tools.bash` uses `/bin/bash -c` in both modes. Restricted mode
...
- `Runtime.ensure_mcp_server()` starts `openterm-tools.service` with `systemctl --user start openterm-tools.service` when the Unix socket is missing. If user systemd is unavailable, it starts `python -m openterm.toolset.server` directly as a detached fallback.
...
- `packaging/openterm-tools.service` is the systemd user service template; the assembled package installs it under `/usr/lib/systemd/user/` and launches `/usr/lib/openterm/openterm-tools`.
- `packaging/openterm-authd.service` is the root systemd service for openterm-authd; the assembled package installs it under `/usr/lib/systemd/system/` and launches `/usr/lib/openterm/openterm-authd`.
- `packaging/openterm-authd.py` owns the root-side authd protocol: password registration is peer-UID-bound and rate-limited, tokens are stored only as SHA-256 digests with a TTL, and the authd socket is `/run/openterm/authd.sock`.
- Privileged whitelist state lives in the singleton in `openterm/toolset/config.py`.
...
- Use current import paths in tests. Patch `openterm.core.agent.chat_with_model_api`, `openterm.core.runtime.FastMCPClient`, and `openterm.toolset.tools.*` rather than stale `openterm.llm`, `openterm.runtime`, or `openterm.toolset.tools_mcp` paths.
