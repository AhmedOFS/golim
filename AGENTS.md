...
- `cterm/app_home.py`: resolves the per-user application home at `~/.cterm` and binds it to the current context. Config, model history, prompt history, skills, transcripts, logs, and the privileged whitelist use this home.
...
- Application-home resolution is shared through `cterm/app_home.py`; preserve `~/.cterm` and the context-bound behavior when changing config, history, skills, transcript, logging, or MCP state paths.
