...
- Unrestricted mode runs commands through `/bin/bash -c` with normal shell syntax when `unrestricted_mode` is true. Before execution, unrestricted mode scans common `sudo <binary>` forms and returns `approval_required` when the resolved binary is not whitelisted.
