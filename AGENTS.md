...
- `openterm/core/runtime.py`: owns MCP client lifecycle, tool discovery, skill selection, interrupt state, and run state copied from the agent (`messages`, `execution_history`, `result`, `last_thinking_trace`). It starts `openterm-mcp.service` with `systemctl --user start` when the socket is missing, falls back to launching `openterm.mcp.server` directly when user systemd is unavailable, retries tool discovery, optionally performs proactive sudo authentication according to config, and exposes `run()` plus `run_followup()`. Followups are user inputs starting with `//`: after a completed run they inject `followup: ...`; while a run is active the TUI queues them as an interrupt/elaboration and injects `clarification: ...`.
...
- Keep MCP server responses newline-delimited JSON. Streaming tool calls send zero or more `tools/progress` notifications (JSON-RPC frames with a `method` and no `id`, correlated by `params.token`) followed by one final `"result"` frame. A held privileged bash call may also send an `approval_request` frame, resolved out-of-band by a separate `approval/respond` request, or an `auth_request` frame when the session token is missing, resolved by `auth/respond` carrying the password for broker registration. Approval and auth exchanges are never returned to the model as a tool result, and passwords never appear in server logs (raw request text is redacted before logging).
- The privileged wrapper is covered by a wrapper-specific `NOPASSWD` sudoers rule, but it is not an unrestricted root escape: the root-side `openterm-broker` service verifies a startup sudo password and mints a random in-memory session token. The MCP server passes that token only through `OPENTERM_SESSION_TOKEN`; the wrapper verifies it with the broker on every invocation and fails closed if the broker, token, or whitelist check fails. Passwords are never passed through command stdin, so ordinary pipes and PTY-backed apt/snap commands remain independent of authentication. Before any wrapper invocation the bash tool consults an injected `_session_token` provider; a cached token short-circuits, otherwise one out-of-band `auth_request` collects the password and registers it with the broker.
...
- `openterm.mcp.tools.bash` uses `/bin/bash -c` in both modes. Restricted mode
...
- `packaging/openterm-broker.service` is the root systemd service for the token broker; the assembled package installs it under `/usr/lib/systemd/system/` and launches `/usr/lib/openterm/openterm-broker`.
- `packaging/openterm-broker.py` owns the root-side broker protocol: password registration is peer-UID-bound and rate-limited, tokens are stored only as SHA-256 digests with a TTL, and the broker socket is `/run/openterm/broker.sock`.
