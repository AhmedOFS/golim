...
- `cterm/config.py`: `Config` class reading/writing `~/.config/cterm/config.json` (or `$XDG_CONFIG_HOME/cterm/config.json`). Keys: `selected_model`, `small_model`, `bash_unrestricted`, `api_provider` (ollama/openrouter/llamacpp), `openrouter_api_key`, `openrouter_model`, `openrouter_small_model`, `llamacpp_server_url`, `llamacpp_model`, `llamacpp_small_model`.
