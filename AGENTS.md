...
- Continuation is part of the observable TUI behavior and is the default. A plain message after a completed run preserves previous `Runtime.messages` and `Runtime.execution_history` and appends a `followup: ...` user message. A plain message while a run is busy queues a pending followup, interrupts the active run via `Runtime.interrupt()`, and continues with `clarification: ...`. Slash commands are the intentional non-conversation actions, detected by `GolimApp.detect_slash_command`: `/new` empties the transcript and resets the runtime conversation via `Runtime.reset_conversation()` without starting a run, so the next message is handled as a new command. There is no `//` prefix.
...
- Prompt placeholder text reflects the continuation default: initial `What do you want to do...`, running `Type a clarification for the current run...`, and completed `Type to continue, or /new to start a new chat...`. Keep these as input placeholders rather than footer text.
