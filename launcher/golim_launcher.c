...
#ifndef GOLIM_ENTRY_MODULE
#define GOLIM_ENTRY_MODULE "golim"
#endif
...
static int join_path(char *buffer, size_t size, const char *directory,
    ...
    if (written < 0 || (size_t)written >= size) {
        fprintf(stderr, "golim launcher: runtime path is too long\n");
        return -1;
    }
    return 0;
}
...
static int executable_path(char *buffer, size_t size) {
    ...
    if (_NSGetExecutablePath(unresolved, &length) != 0 ||
        ...
        fprintf(stderr, "golim launcher: cannot locate executable: %s\n",
                strerror(errno));
        return -1;
    }
    return 0;
...
static int bundled_python(const char *runtime, char *buffer, size_t size) {
    ...
    if (directory == NULL) {
        fprintf(stderr, "golim launcher: cannot open bundled bin directory: %s\n",
                strerror(errno));
        return -1;
    }
    ...
    while ((entry = readdir(directory)) != NULL) {
        ...
        if (access(candidate, X_OK) == 0) {
            ...
            closedir(directory);
            if (written < 0 || (size_t)written >= size) {
                fprintf(stderr, "golim launcher: interpreter path is too long\n");
                return -1;
            }
            return 0;
        }
    ...
    closedir(directory);
    fprintf(stderr, "golim launcher: bundled Python interpreter was not found\n");
    return -1;
}
...
int main(int argc, char **argv) {
    ...
    if (last_slash == NULL) {
        fprintf(stderr, "golim launcher: executable has no parent directory\n");
        return 1;
    }
    *last_slash = '\0';
    ...
    if (written < 0 || (size_t)written >= sizeof(python_path)) {
        fprintf(stderr, "golim launcher: PYTHONPATH is too long\n");
        return 1;
    }
    if (setenv("PYTHONPATH", python_path, 1) != 0) {
        fprintf(stderr, "golim launcher: cannot set PYTHONPATH: %s\n",
                strerror(errno));
        return 1;
    }
    ...
    child_argv[0] = python;
    child_argv[1] = "-m";
    child_argv[2] = GOLIM_ENTRY_MODULE;
    for (int index = 1; index < argc; index++) {
        child_argv[index + 3] = argv[index];
    }
    child_argv[argc + 3] = NULL;
    execv(python, child_argv);
    fprintf(stderr, "golim launcher: cannot execute Python: %s\n",
            strerror(errno));
    return 1;
}
...
#ifndef GOLIM_ENTRY_MODULE
#define GOLIM_ENTRY_MODULE "golim.__main__"
#endif
#ifndef GOLIM_ENTRY_FUNCTION
#define GOLIM_ENTRY_FUNCTION "main"
#endif
...
static void print_status(const char *operation, PyStatus status) {
    ...
    if (PyStatus_IsError(status)) {
        fprintf(stderr, "golim launcher: %s: %s\n", operation,
...
static int find_stdlib(const char *runtime, char *buffer, size_t size) {
    ...
    if (directory == NULL) {
        fprintf(stderr, "golim launcher: cannot open bundled lib directory: %s\n",
                strerror(errno));
        return -1;
    }
    ...
    while ((entry = readdir(directory)) != NULL) {
        ...
        if (stat(candidate, &details) == 0 && S_ISDIR(details.st_mode)) {
            ...
            closedir(directory);
            if (written < 0 || (size_t)written >= size) {
                fprintf(stderr, "golim launcher: standard library path is too long\n");
                return -1;
            }
            return 0;
        }
    ...
    closedir(directory);
    fprintf(stderr, "golim launcher: bundled standard library was not found\n");
    return -1;
}
...
static int append_search_path(PyConfig *config, const char *path) {
    ...
    if (wide_path == NULL) {
        fprintf(stderr, "golim launcher: cannot decode module search path\n");
        return -1;
    }
...
int main(int argc, char **argv) {
    ...
    *last_slash = '\0';
    ...
    PyObject *module = PyImport_ImportModule(GOLIM_ENTRY_MODULE);
    if (module == NULL) {
        PyErr_Print();
        Py_FinalizeEx();
        return 1;
    }
    PyObject *main_function = PyObject_GetAttrString(module, GOLIM_ENTRY_FUNCTION);
    Py_DECREF(module);
